...
未登入訪客第一次建立團購訂單時，送單內容必須另含 `edit_code`，格式為 6 碼數字。後端只保存 SHA-256 雜湊，不保存、回傳或記錄原始修改碼。登入使用者不需要此欄位；店家固定菜單訂單也不套用此機制。

換裝置時可呼叫 `POST /api/groups/{public_code}/orders/recover`，送出正規化前的 `contact_method`、`contact_value` 與 `edit_code`。驗證成功只回傳該張訂單的編號、姓名、金額與明細，前端將內容放回購物車；更新時再以 `repeat_action: "replace"` 送至原送單 API，因此沿用同一訂單編號並由後端重新核價。

只知道手機或 Email、修改碼錯誤、或歷史訂單的修改碼雜湊為 `NULL` 時，一律不回傳訂單內容。修改碼失敗狀態只在單一服務記憶體保存不可逆來源／身分雜湊；同一來源與身分 10 分鐘內第 5 次失敗回傳 HTTP `429` 與 `Retry-After`。服務重新啟動會清除記憶體限制。
